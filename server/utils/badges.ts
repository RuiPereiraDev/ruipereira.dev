...
}, {name: 'spigot/downloads', getKey: (_: H3Event, pluginID: number) => pluginID.toString(), maxAge: 60 * 60})
...
}, {name: 'hangar/downloads', getKey: (_: H3Event, pluginID: number) => pluginID.toString(), maxAge: 60 * 60})
...
}, {name: 'modrinth/downloads', getKey: (_: H3Event, pluginID: number) => pluginID.toString(), maxAge: 60 * 60})
...
export const cachedGitHubDownloads = defineCachedFunction(async (_: H3Event, repository: string): Promise<number> => {
    ...
    return downloads
}, {name: 'github/downloads', getKey: (_: H3Event, repository: string) => repository, maxAge: 60 * 60})
...
export const cachedGitHubRelease = defineCachedFunction(async (_: H3Event, repository: string): Promise<string> => {
    ...
    return release.name
}, {name: 'github/release', getKey: (_: H3Event, repository: string) => repository, maxAge: 60 * 60})
...
export const defineBadgeEventHandler = <T extends EventHandlerRequest, D>(
    ...
    try {
        ...
        setResponseHeader(event, 'cache-control', 'public, must-revalidate, proxy-revalidate, max-age=3600')
        return response
    } catch (err) {
